using LoginService.Api.Extensions;
using LoginService.Api.Middlewares;
using LoginService.Api.ModelBinders;
...
.AddJsonOptions(o =>
{
    // Estandarizar respuestas en camelCase para coincidir con login-node
    o.JsonSerializerOptions.PropertyNamingPolicy = System.Text.Json.JsonNamingPolicy.CamelCase;
});

// Configure services through extension methods
builder.Services.AddScoped<LoginService.Application.Interfaces.ILogService, LoginService.Application.Services.LogService>();
    ...
    try
    {
        ...
        if (addresses != null && addresses.Any())
        {
            foreach (var addr in addresses)
            {
                ...
                startupLogger.LogInformation("API de LoginService está ejecutándose en {Url}. Endpoint de salud: {HealthUrl}", addr, health);
            }
        }
        else
        {
            startupLogger.LogInformation("API de LoginService iniciada. Endpoint de salud: /api/v1/health");
        }
    }
    catch (Exception ex)
    {
